...
//       — full Finnhub + Yahoo signal analysis
...
// ── Analyst price target — 5 sources ─────────────────────────────────────────
async function getAnalyst(ticker) {
  // Source 1: Finnhub price-target
  try {
    const d = await fh(`/stock/price-target?symbol=${ticker}`);
    const t = d?.targetMedian || d?.targetMean || d?.targetHigh;
    if (t && t > 0) return t;
  } catch(_) {}
  ...
  // Source 4: Yahoo v7 quote (sometimes includes targetMeanPrice directly)
  try {
    const r = await fetch(
      `https://query1.finance.yahoo.com/v7/finance/quote?symbols=${ticker}&fields=targetMeanPrice,targetMedianPrice,targetHighPrice`,
      { headers:{'User-Agent':'Mozilla/5.0'}, signal:AbortSignal.timeout(5000) }
    );
    if (r.ok) {
      const j = await r.json();
      const q = j?.quoteResponse?.result?.[0];
      const t = q?.targetMedianPrice || q?.targetMeanPrice || q?.targetHighPrice;
      if (t && t > 0) return t;
    }
  } catch(_) {}
 
  // Source 5: Yahoo quoteSummary with recommendationTrend to confirm coverage exists,
  // then grab target from a broader module set
  try {
    const r = await fetch(
    ...
    if (r.ok) {
      ...
      const t = res?.financialData?.targetMedianPrice?.raw
             || res?.financialData?.targetMeanPrice?.raw
             || res?.summaryDetail?.targetMedianPrice?.raw;
      if (t && t > 0) return t;
    }
  } catch(_) {}
...
// ── Insider transactions — cross-referenced from 3 sources ───────────────────
// Returns a merged, deduplicated set of buys/sells with source labelling
async function getInsider(ticker, px) {
  const now = Math.floor(Date.now() / 1000);
  const cutoff = new Date((now - 90 * 86400) * 1000); // 90 day window
  const from = cutoff.toISOString().slice(0, 10);
  const to = new Date(now * 1000).toISOString().slice(0, 10);
 
  const allBuys = [], allSells = [];
  const seenDates = new Set(); // dedup by date+share count
 
  // Source 1: Yahoo insiderTransactions
  try {
    const r = await fetch(
  ...
  // Source 2: Finnhub insider transactions
  try {
    ...
    for (const t of txns) {
      ...
      const key = `${t.transactionDate}-${sh}`;
      if (seenDates.has(key)) continue;
      seenDates.add(key);
  ...
  // Source 3: SEC EDGAR Form 4 — only if still no data
  if (!allBuys.length && !allSells.length) {
    try {
      ...
      if (r.ok) {
        const j = await r.json();
        const hits = (j?.hits?.hits || []).filter(h =>
          (h._source?.form_type || '').toUpperCase() === '4' &&
          new Date(h._source?.file_date) >= cutoff
        );
        if (hits.length) {
          // EDGAR doesn't tell us buy vs sell without parsing XML — mark as activity
          return { buys: hits.slice(0,5).map(h => ({
            transactionDate: h._source.file_date, share:0, value:0, transactionPrice:px
          })), sells:[], src:'sec' };
        }
      }
    } catch(_) {}
  ...
  if (allBuys.length || allSells.length) {
    return { buys:allBuys, sells:allSells, src: allBuys.length ? 'multi' : 'multi-sell' };
  }
...
function iSig(buys, sells, src) {
  if (buys.length) {
    if (src === 'sec') {
      const rc = ago(buys[0].transactionDate);
      return { status:'pass', value:`Form 4 activity · ${buys.length} filing${buys.length>1?'s':''}${rc?' · '+rc:''}` };
    }
    ...
    return { status:'pass', value:[`${buyCount} buy${buyCount>1?'s':''}`, sh, dl, rc].filter(Boolean).join(' · ') };
  }
  if (sells.length) {
    const rc = ago(sells.map(t=>t.transactionDate).sort().reverse()[0]);
    // Only mark fail if recent and significant
    const recentSells = sells.filter(s => {
      const d = new Date(s.transactionDate);
      return (Date.now() - d.getTime()) < 30 * 86400000;
    });
    if (recentSells.length === 0) {
      return { status:'neutral', value:`${sells.length} sell${sells.length>1?'s':''} (>30d ago), no recent buys` };
    }
...
async function analyse(ticker) {
  ...
  try {
    ...
    if (earns.length && earns[0].actual!=null && earns[0].estimate!=null) {
      ...
      const r = await fetch(`https://query1.finance.yahoo.com/v10/finance/quoteSummary/${ticker}?modules=earningsHistory`,
        { headers:{'User-Agent':'Mozilla/5.0'}, signal:AbortSignal.timeout(5000) });
      if (r.ok) {
        const j=await r.json(), hist=j?.quoteSummary?.result?.[0]?.earningsHistory?.history||[];
        if (hist.length) {
  ...
  try {
    ...
    if (pe&&eps>0&&hi&&lo) {
      ...
      const r = await fetch(`https://query1.finance.yahoo.com/v10/finance/quoteSummary/${ticker}?modules=summaryDetail,defaultKeyStatistics`,
        { headers:{'User-Agent':'Mozilla/5.0'}, signal:AbortSignal.timeout(5000) });
      if (r.ok) {
        const j=await r.json(), sd=j?.quoteSummary?.result?.[0]?.summaryDetail||{}, ks=j?.quoteSummary?.result?.[0]?.defaultKeyStatistics||{};
  ...
  if (ma&&px) {
    const pct=((px-ma)/ma*100).toFixed(1);
    s3 = px<=ma ? {status:'pass',value:`$${px.toFixed(2)} ≤ MA $${ma.toFixed(2)} (${pct}%)`}
                : {status:'fail',value:`$${px.toFixed(2)} > MA $${ma.toFixed(2)} (+${pct}%)`};
  }
  ...
  try {
    ...
    if (peers.length >= 2) {
      ...
      const pes = pm.filter(r=>r.status==='fulfilled')
        .map(r=>r.value?.metric?.peBasicExclExtraTTM||r.value?.metric?.peTTM)
        .filter(pe=>pe&&pe>0&&pe<300);
      if (pes.length >= 2) {
        const avg=pes.reduce((a,b)=>a+b,0)/pes.length;
        const tpe=m.peBasicExclExtraTTM||m.peTTM;
        if (tpe&&avg) {
          const diff=((tpe-avg)/avg*100);
  ...
  const summary=score>=5?`Strong value candidate — ${score}/6 signals pass. Strengths: ${passes.join(', ')}.`
    :score===4?`Good signals (4/6). Passes: ${passes.join(', ')}.`
    :score===3?`Moderate signals (3/6). Passes: ${passes.join(', ')}.`
    :score>0?`Weak signals (${score}/6). Passes: ${passes.join(', ')}. Fails: ${fails.join(', ')}.`
    :`No signals pass. Fails: ${fails.join(', ')}.`;
 
  const rawEx=(p.exchange||'').replace(/NASDAQ.*/i,'NASDAQ').replace(/New York Stock Exchange.*/i,'NYSE').toUpperCase().trim();
  return {
    ticker, company:p.name||ticker, exchange:rawEx||XM[ticker]||'NYSE',
    price:`$${px.toFixed(2)}`, change:q.dp!=null?`${q.dp>0?'+':''}${q.dp.toFixed(2)}%`:null,
