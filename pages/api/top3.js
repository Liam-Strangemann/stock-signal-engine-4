...
//       — runs full Finnhub signal analysis on 3 pre-selected tickers
...
async function fh(path) {
  ...
  const r = await fetch(`${FH}${path}${sep}token=${FINNHUB_KEY}`, { headers:{'User-Agent':'Mozilla/5.0'}, signal:AbortSignal.timeout(6000) });
...
async function getInsider(ticker,px){
  const now=Math.floor(Date.now()/1000),cutoff=new Date((now-60*86400)*1000),from=cutoff.toISOString().slice(0,10),to=new Date(now*1000).toISOString().slice(0,10);
  try{const r=await fetch(`https://query1.finance.yahoo.com/v10/finance/quoteSummary/${ticker}?modules=insiderTransactions`,{headers:{'User-Agent':'Mozilla/5.0'},signal:AbortSignal.timeout(5000)});if(r.ok){const j=await r.json(),txns=j?.quoteSummary?.result?.[0]?.insiderTransactions?.transactions||[];const b=[],s=[];for(const t of txns){const ts=t.startDate?.raw;if(!ts||new Date(ts*1000)<cutoff)continue;const ds=new Date(ts*1000).toISOString().slice(0,10),sh=Math.abs(t.shares?.raw||0),val=Math.abs(t.value?.raw||0),desc=(t.transactionDescription||'').toLowerCase(),e={transactionDate:ds,share:sh,value:val,transactionPrice:sh>0?val/sh:px};if(/purchase|buy/i.test(desc))b.push(e);else if(/sale|sell/i.test(desc))s.push(e);}if(b.length||s.length)return{buys:b,sells:s,src:'yahoo'};}}catch(_){}
  try{const d=await fh(`/stock/insider-transactions?symbol=${ticker}&from=${from}&to=${to}`);const b=(d?.data||[]).filter(t=>t.transactionCode==='P'),s=(d?.data||[]).filter(t=>t.transactionCode==='S');if(b.length||s.length)return{buys:b,sells:s,src:'fh'};}catch(_){}
  try{const r=await fetch(`https://efts.sec.gov/LATEST/search-index?q=%22${ticker}%22&dateRange=custom&startdt=${from}&enddt=${to}&forms=4`,{headers:{'User-Agent':'signal-engine/1.0'},signal:AbortSignal.timeout(5000)});if(r.ok){const j=await r.json(),hits=(j?.hits?.hits||[]).filter(h=>(h._source?.form_type||'').toUpperCase()==='4'&&new Date(h._source?.file_date)>=cutoff);if(hits.length)return{buys:hits.slice(0,5).map(h=>({transactionDate:h._source.file_date,share:0,value:0,transactionPrice:px})),sells:[],src:'sec'};}}catch(_){}
  return{buys:[],sells:[],src:null};
}
 
function iSig(buys,sells,src){
  if(buys.length){if(src==='sec'){const rc=ago(buys[0].transactionDate);return{status:'pass',value:`Form 4 · ${buys.length} filing${buys.length>1?'s':''}${rc?' · '+rc:''}`};}const sh=fS(buys.reduce((s,t)=>s+(t.share||0),0)),dl=fD(buys.reduce((s,t)=>s+(t.value||0),0)),rc=ago(buys.map(t=>t.transactionDate).sort().reverse()[0]);return{status:'pass',value:[`${buys.length} buy${buys.length>1?'s':''}`,sh,dl,rc].filter(Boolean).join(' · ')};}
...
async function getAnalyst(ticker){
  ...
  for(const h of['query1','query2']){try{const r=await fetch(`https://${h}.finance.yahoo.com/v10/finance/quoteSummary/${ticker}?modules=financialData`,{headers:{'User-Agent':'Mozilla/5.0'},signal:AbortSignal.timeout(5000)});if(r.ok){const j=await r.json(),fd=j?.quoteSummary?.result?.[0]?.financialData,t=fd?.targetMedianPrice?.raw||fd?.targetMeanPrice?.raw;if(t>0)return t;}}catch(_){}}
  return null;
}
 
async function getMA(ticker){
  try{const now=Math.floor(Date.now()/1000),r=await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${ticker}?interval=1d&period1=${now-80*86400}&period2=${now}`,{headers:{'User-Agent':'Mozilla/5.0'},signal:AbortSignal.timeout(5000)});if(r.ok){const j=await r.json(),c=j?.chart?.result?.[0]?.indicators?.quote?.[0]?.close?.filter(x=>x!=null&&!isNaN(x));if(c?.length>=10){const s=c.slice(-50);return s.reduce((a,b)=>a+b,0)/s.length;}}}catch(_){}
...
async function analyse(ticker){
  const[quote,metrics,earnings,profile]=await Promise.allSettled([fh(`/quote?symbol=${ticker}`),fh(`/stock/metric?symbol=${ticker}&metric=all`),fh(`/stock/earnings?symbol=${ticker}&limit=4`),fh(`/stock/profile2?symbol=${ticker}`)]);
  const q=quote.status==='fulfilled'?quote.value||{}:{},m=metrics.status==='fulfilled'?metrics.value?.metric||{}:{},p=profile.status==='fulfilled'?profile.value||{}:{};
  const px=q.c;if(!px)return null;
  const[ma,insider,analystTgt]=await Promise.all([getMA(ticker),getInsider(ticker,px),getAnalyst(ticker)]);
  const mc=p.marketCapitalization?p.marketCapitalization*1e6:0,mcs=mc>1e12?`$${(mc/1e12).toFixed(2)}T`:mc>1e9?`$${(mc/1e9).toFixed(1)}B`:mc>1e6?`$${(mc/1e6).toFixed(0)}M`:'';
 
  let s1={status:'neutral',value:'No data'};
  try{const earns=Array.isArray(earnings.value)?earnings.value:[];if(earns.length&&earns[0].actual!=null&&earns[0].estimate!=null){const diff=earns[0].actual-earns[0].estimate,beat=diff>=0,ds=Math.abs(diff)<0.005?'in-line':beat?`+$${Math.abs(diff).toFixed(2)}`:`-$${Math.abs(diff).toFixed(2)}`;s1={status:beat?'pass':'fail',value:beat?`Beat by ${ds}`:`Missed ${ds}`};}
  else{const r=await fetch(`https://query1.finance.yahoo.com/v10/finance/quoteSummary/${ticker}?modules=earningsHistory`,{headers:{'User-Agent':'Mozilla/5.0'},signal:AbortSignal.timeout(5000)});if(r.ok){const j=await r.json(),hist=j?.quoteSummary?.result?.[0]?.earningsHistory?.history||[];if(hist.length){const rec=hist[hist.length-1],a=rec?.epsActual?.raw,e=rec?.epsEstimate?.raw;if(a!=null&&e!=null){const diff=a-e,beat=diff>=0,ds=Math.abs(diff)<0.005?'in-line':beat?`+$${Math.abs(diff).toFixed(2)}`:`-$${Math.abs(diff).toFixed(2)}`;s1={status:beat?'pass':'fail',value:beat?`Beat by ${ds}`:`Missed ${ds}`};}}}}}catch(_){}
 
  let s2={status:'neutral',value:'No data'};
  try{const pe=m.peBasicExclExtraTTM||m.peTTM,eps=m.epsBasicExclExtraAnnual||m.epsTTM,hi=m['52WeekHigh'],lo=m['52WeekLow'];if(pe&&eps>0&&hi&&lo){const h=((hi+lo)/2)/eps;if(pe<h*0.92)s2={status:'pass',value:`PE ${pe.toFixed(1)}x < hist ~${h.toFixed(0)}x`};else if(pe>h*1.08)s2={status:'fail',value:`PE ${pe.toFixed(1)}x > hist ~${h.toFixed(0)}x`};else s2={status:'neutral',value:`PE ${pe.toFixed(1)}x ≈ hist ~${h.toFixed(0)}x`};}else if(pe)s2={status:'neutral',value:`PE ${pe.toFixed(1)}x`};
  else{const r=await fetch(`https://query1.finance.yahoo.com/v10/finance/quoteSummary/${ticker}?modules=summaryDetail,defaultKeyStatistics`,{headers:{'User-Agent':'Mozilla/5.0'},signal:AbortSignal.timeout(5000)});if(r.ok){const j=await r.json(),sd=j?.quoteSummary?.result?.[0]?.summaryDetail||{},ks=j?.quoteSummary?.result?.[0]?.defaultKeyStatistics||{},pe2=sd?.trailingPE?.raw||sd?.forwardPE?.raw,eps2=ks?.trailingEps?.raw,hi2=sd?.fiftyTwoWeekHigh?.raw,lo2=sd?.fiftyTwoWeekLow?.raw;if(pe2&&eps2&&hi2&&lo2){const h=((hi2+lo2)/2)/eps2;if(pe2<h*0.92)s2={status:'pass',value:`PE ${pe2.toFixed(1)}x < hist ~${h.toFixed(0)}x`};else if(pe2>h*1.08)s2={status:'fail',value:`PE ${pe2.toFixed(1)}x > hist ~${h.toFixed(0)}x`};else s2={status:'neutral',value:`PE ${pe2.toFixed(1)}x ≈ hist ~${h.toFixed(0)}x`};}else if(pe2)s2={status:'neutral',value:`PE ${pe2.toFixed(1)}x`};}}}catch(_){}
  ...
  try{const pd=await fh(`/stock/peers?symbol=${ticker}`);const peers=Array.isArray(pd)?pd.filter(x=>x!==ticker).slice(0,8):[];if(peers.length>=2){const pm=await Promise.allSettled(peers.map(p=>fh(`/stock/metric?symbol=${p}&metric=all`)));const pes=pm.filter(r=>r.status==='fulfilled').map(r=>r.value?.metric?.peBasicExclExtraTTM||r.value?.metric?.peTTM).filter(pe=>pe&&pe>0&&pe<300);if(pes.length>=2){const avg=pes.reduce((a,b)=>a+b,0)/pes.length,tpe=m.peBasicExclExtraTTM||m.peTTM;if(tpe&&avg){const diff=((tpe-avg)/avg*100);if(diff<-8)s6={status:'pass',value:`${Math.abs(diff).toFixed(0)}% < peer avg ${avg.toFixed(1)}x`};else if(diff>8)s6={status:'fail',value:`${Math.abs(diff).toFixed(0)}% > peer avg ${avg.toFixed(1)}x`};else s6={status:'neutral',value:`In line, avg ${avg.toFixed(1)}x`};}}}}catch(_){}
  ...
  const passes=signals.map((s,i)=>s.status==='pass'?N[i]:null).filter(Boolean),fails=signals.map((s,i)=>s.status==='fail'?N[i]:null).filter(Boolean);
