// pages/api/top3.js
// Phase 1: quick 2-call scan across 150 watchlist stocks to find top candidates
// Phase 2: full analyse (all 6 signals) on top 6 candidates only
// Result: top 3 fully-detailed stocks, well within free tier limits
...
const WATCHLIST = [
  ...
  'HD','MCD','NKE','SBUX','LOW','TGT','COST','BKNG','MAR','HLT','YUM','DPZ',
  'WMT','KO','PEP','PG','MDLZ','CL','KMB','GIS',
  'XOM','CVX','COP','EOG','SLB','MPC','PSX','VLO','OXY','DVN','HAL','BKR',
  ...
  'MO','PM','IBM',
].filter(function(v,i,a){ return a.indexOf(v)===i; }).slice(0,120);
...
// Quick 2-call screen: quote + metric only
async function quickScreen(ticker) {
  try {
    ...
    // PE below 52wk midpoint historical
    ...
    // Price at or below 50d MA
    const ma50 = m['50DayMA'] || m['50DayMovingAvg'];
    if (ma50 && px <= ma50) score++;
 
    // EPS quarterly growth positive
    const qg = m.epsGrowthQuarterlyYoy;
    if (qg != null && qg > 0) score++;
 
    // PE reasonably low vs market
    if (curPE && curPE > 0 && curPE < 20) score++;
 
    return { ticker: ticker, score: score, px: px };
  } catch(_) { return null; }
}
 
// Full analyse for top candidates -- mirrors analyse.js logic
async function maFromFinnhub(ticker) {
  ...
  try {
    ...
    if (r.ok) {
      ...
      const closes = (j&&j.chart&&j.chart.result&&j.chart.result[0]&&j.chart.result[0].indicators&&j.chart.result[0].indicators.quote&&j.chart.result[0].indicators.quote[0]&&j.chart.result[0].indicators.quote[0].close)||[];
...
async function analystTarget(ticker) {
  try {
    ...
    if (r.ok) {
      ...
      const t = (fd&&fd.targetMedianPrice&&fd.targetMedianPrice.raw)||(fd&&fd.targetMeanPrice&&fd.targetMeanPrice.raw);
      if (t && t > 0) return t;
    }
  } catch(_) {}
  return null;
...
async function fullAnalyse(ticker) {
  try {
    ...
    try {
      ...
      if (earns.length>0) {
        const e=earns[0]; const diff=e.actual-e.estimate; const beat=diff>=0;
    ...
    try {
      ...
      if (maVal&&px) {
        ...
        s3=px<=maVal?{status:'pass',value:'$'+px.toFixed(2)+' <= MA $'+maVal.toFixed(2)+' ('+pct+'%)'}:{status:'fail',value:'$'+px.toFixed(2)+' > MA $'+maVal.toFixed(2)+' (+'+pct+'%)'};
      }
    } catch(_){}
    ...
    try {
      ...
      if (buys.length>0) {
        ...
        const recency=days!=null?(days===0?'today':days===1?'1d ago':days<7?days+'d ago':Math.floor(days/7)+'w ago'):null;
        s4={status:'pass',value:buys.length+' buy'+(buys.length>1?'s':'')+' '+fv+(recency?' - '+recency:'')};
    ...
    try {
      ...
      if (tgt&&px) {
        ...
        s5=parseFloat(up)>=25?{status:'pass',value:'Target $'+tgt.toFixed(2)+', +'+up+'% upside'}:{status:'fail',value:'Target $'+tgt.toFixed(2)+', +'+up+'% upside'};
      }
    } catch(_){}
 
    // s6 peer PE (lightweight - skip for top3 to save API calls, show neutral)
    ...
    const passes=signals.map(function(s,i){return s.status==='pass'?['EPS beat','Low PE','Below 50d MA','Insider buying','Analyst upside','PE vs peers'][i]:null;}).filter(Boolean);
    const fails=signals.map(function(s,i){return s.status==='fail'?['EPS beat','Low PE','Below 50d MA','Insider buying','Analyst upside','PE vs peers'][i]:null;}).filter(Boolean);
...
export default async function handler(req, res) {
  ...
  try {
    // Phase 1: quick screen in batches of 20
    const BATCH = 20;
    const allQuick = [];
    for (let i=0; i<WATCHLIST.length; i+=BATCH) {
      const batch = WATCHLIST.slice(i, i+BATCH);
      const results = await Promise.allSettled(batch.map(quickScreen));
      results.forEach(function(r){
        if (r.status==='fulfilled'&&r.value) allQuick.push(r.value);
      });
      if (i+BATCH < WATCHLIST.length) await sleep(1500);
    }
 
    // Sort by quick score, take top 6 candidates for full analysis
    allQuick.sort(function(a,b){return b.score-a.score;});
    const candidates = allQuick.slice(0,6).map(function(c){return c.ticker;});
 
    // Phase 2: full analyse on top 6 candidates (parallel, ~30 API calls)
    ...
    return res.status(200).json({top3, scannedAt:new Date().toISOString(), totalScanned:allQuick.length});
  } catch(e) {
