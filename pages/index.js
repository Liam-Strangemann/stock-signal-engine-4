...
// ── Universe scanned client-side via Yahoo Finance ────────────────────────────
const UNIVERSE = [
...
// Quick-score a stock from Yahoo data — returns null if it fails quality gate
...
// Fetch one Yahoo quote — runs in browser, no CORS issues on Yahoo's v8 endpoint
...
// Scan all stocks in parallel batches of 20
async function scanUniverse(onProgress) {
  const BATCH = 20;
  const results = [];
  for (let i = 0; i < UNIQ_UNIVERSE.length; i += BATCH) {
    const batch = UNIQ_UNIVERSE.slice(i, i+BATCH);
    const batchResults = await Promise.all(batch.map(fetchYahooQuote));
    results.push(...batchResults.filter(Boolean));
    if (onProgress) onProgress(results.length, UNIQ_UNIVERSE.length);
    // Small pause between batches to avoid rate-limiting
    if (i + BATCH < UNIQ_UNIVERSE.length) await new Promise(r => setTimeout(r, 50));
  }
  return results;
}
...
function getExchange(stock) {
  ...
  return EXCHANGE_MAP[stock.ticker] || (US_SET.has(stock.ticker) ? 'NYSE' : 'INTL');
}
 
var C = {
  pageBg:'#F1EFE8',cardBg:'#E8E5DC',darkBg:'#5F5E56',deepBg:'#3A3832',
  accent:'#8B7D6B',accentDk:'#6B5D4F',gold:'#B8A070',
...
function ScoreDots({score,max=6}) {
  return <div style={{display:'flex',gap:4}}>{Array.from({length:max}).map((_,i)=>{var f=i<score,c=score>=5?C.green:score>=4?'#6A8B60':score>=3?'#B8903A':C.txLight;return <div key={i} style={{width:7,height:7,borderRadius:'50%',background:f?c:'transparent',border:'1px solid '+(f?c:C.borderDk)}}/>;})}</div>;
}
...
function ScoreDisplay({score,size=22}) {
  var sc=Math.min(score||0,6),c=sc>=5?C.gold:sc>=4?'#A8C080':sc>=3?'#C8A870':C.txLight;
  return <span style={{fontFamily:MONO,fontSize:size,fontWeight:400,letterSpacing:'0.05em',fontVariantNumeric:'tabular-nums',color:c,lineHeight:1}}>{sc}<span style={{opacity:0.5,margin:'0 1px'}}>/</span>6</span>;
}
...
function FeatureCard({stock,rank}) {
  ...
  return (
    <div style={{background:C.deepBg,border:'1px solid '+C.accent,borderTop:'3px solid '+C.gold,borderRadius:2,padding:'24px 22px',flex:1,minWidth:0,display:'flex',flexDirection:'column'}}>
...
function ResultCard({stock,rank}) {
  var sc=Math.min(stock.score||0,6),rating=getRating(sc),chgPos=stock.change?.startsWith('+');
  ...
  return (
    <div style={{background:C.cardBg,border:'0.5px solid '+C.borderDk,borderLeft:'3px solid '+(sc>=5?C.gold:sc>=4?C.green:sc>=3?'#B8903A':C.borderDk),borderRadius:2,padding:'14px 16px'}}>
  ...
  return (
    <div style={{marginTop:12}}>
      <div style={{height:2,background:C.border,borderRadius:1,overflow:'hidden',marginBottom:8}}>
        <div style={{height:'100%',background:C.gold,width:pct+'%',transition:'width 0.3s'}}/>
      </div>
      <div style={{textAlign:'center',fontSize:11,color:C.txLight,fontFamily:MONO,letterSpacing:'0.06em'}}>
        Scanning {scanned} of {total} securities{pct>=100?' — enriching top picks...':''}
      </div>
    </div>
  );
}
...
export default function Home() {
  ...
  var [scanState,setScanState]       = useState('idle'); // idle|scanning|enriching|ready
  var [scanProgress,setScanProgress] = useState({scanned:0,total:UNIQ_UNIVERSE.length});
  var timerRef   = useRef(null);
  var tickersRef = useRef([]);
  var hasRunRef  = useRef(false);
 
  // On mount: check cache first, then do client-side scan
  useEffect(function() {
    if (hasRunRef.current) return;
    hasRunRef.current = true;
 
    // Check server cache first — instant if warm
    fetch('/api/top3')
      .then(r => r.json())
      .then(function(d) {
        if (d.top3 && d.top3.length > 0) {
          setTop3(d); setScanState('ready');
        } else {
          // Cache empty — run client-side scan
          runClientScan();
        }
      })
      .catch(function() { runClientScan(); });
  }, []);
 
  async function runClientScan() {
    setScanState('scanning');
    setScanProgress({scanned:0, total:UNIQ_UNIVERSE.length});
 
    try {
      // Step 1: Scan all stocks in browser — no timeout, no Vercel limit
      const allStocks = await scanUniverse(function(scanned, total) {
        setScanProgress({scanned, total});
      });
 
      // Step 2: Quick-score and pick top 5 candidates
      ...
      setScanState('enriching');
      setScanProgress({scanned:UNIQ_UNIVERSE.length, total:UNIQ_UNIVERSE.length});
 
      // Step 3: POST candidates to server for Finnhub enrichment
      // Server only runs 3 stocks × 4 Finnhub calls = 12 calls, done in <5s
      const res = await fetch('/api/top3', {
        method: 'POST',
        headers: {'Content-Type':'application/json'},
        body: JSON.stringify({ tickers:candidates, totalScanned:allStocks.length }),
      });
  ...
  var scan = useCallback(function(tickers) {
    ...
    return fetch('/api/analyse', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({tickers})})
      ...
      .then(function(data) {
        var arr=Object.values(data.results).filter(Boolean).sort((a,b)=>(b.score||0)-(a.score||0));
        setResults(arr); setUpdatedAt(new Date().toLocaleTimeString()); setStatus(''); setProgress(100);
      })
      .catch(e=>setStatus('Error: '+e.message))
      .finally(function(){setScanning(false);setTimeout(()=>setProgress(0),1000);});
  },[]);
 
  function runScan() {
    var tickers=input.split(/[\s,;]+/).map(t=>t.toUpperCase().trim()).filter(Boolean).slice(0,20);
    if (!tickers.length) return;
    tickersRef.current=tickers; clearInterval(timerRef.current); setResults([]); scan(tickers);
  ...
  function exportCSV() {
    ...
    var rows=filtered.map(function(r,i){var g=r.signals||[];return [i+1,r.ticker,'"'+(r.company||'').replace(/"/g,'""')+'"',r.score||0,r.price||'',r.change||'',r.marketCap||'',g[0]?g[0].value||'':'',g[1]?g[1].value||'':'',g[2]?g[2].value||'':'',g[3]?g[3].value||'':'',g[4]?g[4].value||'':'',g[5]?g[5].value||'':'','"'+(r.summary||'').replace(/"/g,'""')+'"'].join(',');});
    var blob=new Blob([[hdr.join(',')].concat(rows).join('\n')],{type:'text/csv'});
    var a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='signals_'+new Date().toISOString().slice(0,10)+'.csv';a.click();
  }
  ...
  return (
    <>
      <Head>
        ...
        <style>{`*{box-sizing:border-box;margin:0;padding:0}body{background:${C.pageBg}}::selection{background:${C.gold};color:#2C2C2A}input::placeholder{color:${C.txLight}}input:focus{outline:none}button{cursor:pointer}button:disabled{opacity:0.4;cursor:not-allowed}@keyframes fadeUp{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}@keyframes spin{to{transform:rotate(360deg)}}@keyframes shimmer{0%,100%{opacity:0.4}50%{opacity:0.8}}.card-anim{animation:fadeUp 0.4s ease both}.card-anim:nth-child(1){animation-delay:0.05s}.card-anim:nth-child(2){animation-delay:0.12s}.card-anim:nth-child(3){animation-delay:0.19s}`}</style>
      </Head>
        ...
        <div style={{maxWidth:1200,margin:'0 auto',padding:'32px 32px 80px'}}>
          ...
          <div style={{marginBottom:40}}>
            <div style={{display:'flex',alignItems:'baseline',gap:16,marginBottom:20}}>
              ...
              <div style={{fontSize:10,color:C.txLight,fontFamily:SANS,letterSpacing:'0.1em',textTransform:'uppercase',whiteSpace:'nowrap'}}>
                {scanState==='ready'&&scannedTotal>0?scannedTotal+' securities screened':''}
              </div>
            </div>
 
            {/* Shimmer + progress while scanning */}
            {scanState!=='ready'&&(
              <div>
                <div style={{display:'flex',gap:16,marginBottom:12}}>
                  {[0,1,2].map(i=>(
                    <div key={i} style={{flex:1,background:C.cardBg,border:'0.5px solid '+C.border,borderTop:'3px solid '+C.border,borderRadius:2,padding:'24px 22px',animation:'shimmer 1.5s infinite',animationDelay:(i*0.2)+'s'}}>
                      ...
                      <div style={{width:140,height:12,background:C.border,borderRadius:2,marginBottom:6}}/>
                      <div style={{width:100,height:10,background:C.border,borderRadius:2}}/>
                    </div>
                  ))}
                </div>
                {(scanState==='scanning'||scanState==='enriching')&&(
                  <ScanProgress scanned={scanProgress.scanned} total={scanProgress.total}/>
                )}
              </div>
            )}
 
            {scanState==='ready'&&top3Stocks.length>0&&(
          ...
          <div style={{background:C.cardBg,border:'0.5px solid '+C.borderDk,padding:'20px',marginBottom:20}}>
            <div style={{display:'flex',gap:10,marginBottom:14}}>
              <input type="text" value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')runScan();}}
          ...
          <div style={{display:'flex',gap:6,flexWrap:'wrap',alignItems:'center',marginBottom:16}}>
            ...
            {[['all','All'],['strong','Strong 5-6'],['mod','Moderate 3-4'],['weak','Weak 0-2'],['us','US'],['intl','International']].map(kl=>{var active=filter===kl[0];return <button key={kl[0]} onClick={()=>setFilter(kl[0])} style={{padding:'4px 12px',fontSize:10,fontFamily:SANS,letterSpacing:'0.06em',background:active?C.accentDk:'transparent',color:active?'#F1EFE8':C.txMid,border:'0.5px solid '+(active?C.accentDk:C.borderDk),borderRadius:0}}>{kl[1]}</button>;})}
          </div>
