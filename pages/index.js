...
// Quick-score from Yahoo data — picks best candidates to pass to full analysis
...
function getExchange(stock) {
  // analyse.js returns exchange via Finnhub profile; fall back to our map
  return stock.exchange || XM[stock.ticker] || (US_SET.has(stock.ticker) ? 'NYSE' : 'INTL');
}
  ...
  return (
    <div style={{ display:'flex', gap:4 }}>
      {Array.from({ length:max }).map((_,i) => {
        var f=i<score, c=score>=5?C.green:score>=4?'#6A8B60':score>=3?'#B8903A':C.txLight;
        return <div key={i} style={{ width:7, height:7, borderRadius:'50%', background:f?c:'transparent', border:'1px solid '+(f?c:C.borderDk) }}/>;
      })}
    </div>
  );
}
...
function SigPill({ sig }) {
  ...
  return (
    <div style={{ background:bg, border:'0.5px solid '+bd, borderRadius:6, padding:'5px 7px' }}>
      ...
      <div style={{ fontSize:10, fontWeight:500, color, fontFamily:MONO, lineHeight:1.3, wordBreak:'break-word' }}>{sig.value || '--'}</div>
    </div>
  );
}
...
function ScoreDisplay({ score, size=22 }) {
  ...
  return (
    <span style={{ fontFamily:MONO, fontSize:size, fontWeight:400, letterSpacing:'0.05em', fontVariantNumeric:'tabular-nums', color:c, lineHeight:1 }}>
      {sc}<span style={{ opacity:0.5, margin:'0 1px' }}>/</span>6
    </span>
  );
}
 
// Loading status shown under shimmer cards
function ScanStatus({ phase, scanned, total }) {
  ...
  if (phase === 'enriching') return (
    <div style={{ textAlign:'center', fontSize:11, fontFamily:MONO, color:C.txLight, letterSpacing:'0.06em', marginTop:10 }}>
      Running full signal analysis on top candidates…
    </div>
  );
...
function FeatureCard({ stock, rank }) {
  ...
  var medals  = ['I','II','III'];
  var chgPos  = stock.change?.startsWith('+');
    ...
    <div style={{ background:C.deepBg, border:'1px solid '+C.accent, borderTop:'3px solid '+C.gold, borderRadius:2, padding:'24px 22px', flex:1, minWidth:0, display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:5, marginBottom:14 }}>
        {SIG_LABELS.map((label,i) => {
          ...
          return <SigPill key={i} sig={{ status:sig.status, value:sig.value, label }}/>;
        })}
      </div>
...
export default function Home() {
  ...
  // ── Top 3 scan: Yahoo quick-score → pass best tickers to /api/top3 ─────────
  async function runTop3Scan() {
    setScanPhase('scanning');
    setScanProg({ scanned:0, total:UNIQ.length });
 
    // Step 1: Yahoo scan via proxy (one call, all symbols)
    let allStocks = [];
    try {
    ...
    // Safety net if scorer filters everything out
    const pool = scored.length >= 3 ? scored
      : allStocks.filter(s=>s.marketCap>1e9).sort((a,b)=>(b.marketCap||0)-(a.marketCap||0));
 
    // Send top 6 candidates; /api/top3 will run analyse on them and return best 3
    const candidates = pool.length >= 3
      ? pool.slice(0,6).map(s=>s.symbol)
      : ['MSFT','JPM','KO','XOM','JNJ','ABBV']; // hardcoded fallback
 
    setScanPhase('enriching');
 
    // Step 3: /api/top3 calls /api/analyse internally — same logic as custom scan
    try {
  ...
  function runScan() {
    var t = input.split(/[\s,;]+/).map(x=>x.toUpperCase().trim()).filter(Boolean).slice(0,20);
    if (!t.length) return;
    tickersRef.current = t;
    ...
    <div style={{ background:C.pageBg, minHeight:'100vh', color:C.tx, fontFamily:SANS }}>
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'32px 32px 80px' }}>
        ...
        <div style={{ marginBottom:40 }}>
          ...
          {/* Shimmer + progress while loading */}
          {scanPhase !== 'ready' && (
            <div>
        ...
        <div style={{ background:C.cardBg, border:'0.5px solid '+C.borderDk, padding:'20px', marginBottom:20 }}>
          ...
          <div style={{ display:'flex', gap:6, flexWrap:'wrap', alignItems:'center' }}>
            ...
            {Object.keys(PRESETS).map(name => {
              var a = activePreset===name;
              return <button key={name} onClick={()=>{setInput(PRESETS[name]);setActivePreset(name);}} style={{ padding:'4px 12px', fontSize:10, fontFamily:SANS, letterSpacing:'0.06em', background:a?C.darkBg:'transparent', color:a?'#F1EFE8':C.txMid, border:'0.5px solid '+(a?C.darkBg:C.borderDk), borderRadius:0, whiteSpace:'nowrap' }}>{name}</button>;
            })}
          </div>
        </div>
        ...
        <div style={{ display:'flex', gap:6, flexWrap:'wrap', alignItems:'center', marginBottom:16 }}>
          ...
          {[['all','All'],['strong','Strong 5-6'],['mod','Moderate 3-4'],['weak','Weak 0-2'],['us','US'],['intl','International']].map(kl => {
            var a = filter===kl[0];
            return <button key={kl[0]} onClick={()=>setFilter(kl[0])} style={{ padding:'4px 12px', fontSize:10, fontFamily:SANS, letterSpacing:'0.06em', background:a?C.accentDk:'transparent', color:a?'#F1EFE8':C.txMid, border:'0.5px solid '+(a?C.accentDk:C.borderDk), borderRadius:0 }}>{kl[1]}</button>;
          })}
        </div>
        ...
        {filtered.length===0 && !scanning ? (
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {filtered.map((s,i) => <ResultCard key={s.ticker} stock={s} rank={i+1}/>)}
          </div>
        )}
