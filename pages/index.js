    ...
    <div style={{ background:C.deepBg,border:`1px solid ${C.accent}`,borderTop:`3px solid ${isNew?'#7EC87A':C.gold}`,borderRadius:2,padding:'24px 22px',position:'relative',animation:isNew?'fadeUpNew 0.5s ease both':'fadeUp 0.4s ease both',display:'flex',flexDirection:'column',height:'100%' }}>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:5,marginBottom:14 }}>
        {SIG_LABELS.map((label,i)=>{
          ...
          return <SigPill key={i} sig={{ status:sig.status,value:sig.value }} label={label} dark signalIndex={i} loading={sig._loading||false} onRetry={onSignalRetry?(idx)=>onSignalRetry(stock.ticker,idx):undefined}/>;
        })}
      </div>
...
function ArrowBtn({ dir, onClick, disabled }) {
  return (
    <button onClick={onClick} disabled={disabled} aria-label={dir==='left'?'Previous picks':'Next picks'}
  ...
  return (
    <div style={{ display:'flex',gap:6,alignItems:'center' }}>
      {Array.from({ length:total }).map((_,i)=>(
        <button key={i} onClick={()=>onChange(i)} aria-label={`Page ${i+1}`}
    ...
    <div style={{ background:C.cardBg,borderRadius:2,border:`0.5px solid ${C.borderDk}`,borderLeft:`3px solid ${accentL}`,padding:'14px 16px' }}>
      <div style={{ display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:10,marginBottom:10 }}>
        ...
        <div style={{ textAlign:'right',flexShrink:0,display:'flex',flexDirection:'column',alignItems:'flex-end',gap:8 }}>
          <div style={{ fontSize:22,fontWeight:400,fontFamily:MONO,color:scoreColor(sc,false),lineHeight:1 }}>{sc}<span style={{ color:C.txLight }}>/6</span></div>
          <ScoreDots score={sc}/>
        </div>
      </div>
      <div style={{ display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:4,marginBottom:8 }}>
        {SIG_LABELS.map((label,i)=>{
          const sig=(stock.signals||[])[i]||{};
          return <SigPill key={i} sig={{ status:sig.status,value:sig.value }} label={label} signalIndex={i} loading={sig._loading||false} onRetry={onSignalRetry?(idx)=>onSignalRetry(stock.ticker,idx):undefined}/>;
        })}
      </div>
        ...
        <div style={{ marginBottom:16 }}>
          <div style={{ display:'flex',alignItems:'baseline',gap:16,marginBottom:20 }}>
            ...
            <div style={{ fontSize:9.5,color:C.txLight,fontFamily:SANS,letterSpacing:'0.1em',textTransform:'uppercase',whiteSpace:'nowrap' }}>
              {topStatus||`Scanning ~${totalUniverse.toLocaleString()} securities…`}
            </div>
          </div>
