...
function ScanStatus({phase,scanned,total}){
  ...
  if(phase==='enriching')return(<div style={{textAlign:'center',fontSize:11,fontFamily:MONO,color:C.txLight,letterSpacing:'0.06em',marginTop:10}}>Running signal analysis on top candidates…</div>);
  if(phase==='scanning')return(<div style={{marginTop:10}}><div style={{height:2,background:C.borderDk,borderRadius:1,overflow:'hidden',marginBottom:6}}><div style={{height:'100%',background:C.gold,width:pct+'%',transition:'width 0.5s ease',borderRadius:1}}/></div><div style={{display:'flex',justifyContent:'space-between',fontSize:10,fontFamily:MONO,color:C.txLight,letterSpacing:'0.05em'}}><span>Scanning {scanned} of {total} securities</span><span style={{color:C.gold}}>{pct}%</span></div></div>);
  return null;
}
