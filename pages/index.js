...
var EXCHANGE_MAP = {
  ...
  TMUS:'NASDAQ',EQIX:'NASDAQ',BKNG:'NASDAQ',
  'BRK.B':'NYSE',JPM:'NYSE',JNJ:'NYSE',V:'NYSE',PG:'NYSE',UNH:'NYSE',
  HD:'NYSE',MA:'NYSE',XOM:'NYSE',CVX:'NYSE',ABBV:'NYSE',MRK:'NYSE',
  KO:'NYSE',TMO:'NYSE',MCD:'NYSE',ACN:'NYSE',LIN:'NYSE',NEE:'NYSE',
  PM:'NYSE',UNP:'NYSE',IBM:'NYSE',GS:'NYSE',CAT:'NYSE',BA:'NYSE',
  MMM:'NYSE',GE:'NYSE',WMT:'NYSE',TGT:'NYSE',LOW:'NYSE',NKE:'NYSE',
  NVO:'NYSE',SAP:'NYSE',TM:'NYSE',HSBC:'NYSE',SHEL:'NYSE',BHP:'NYSE',
  RIO:'NYSE',TSM:'NYSE',LLY:'NYSE',ORCL:'NYSE',BAC:'NYSE',WFC:'NYSE',
  MS:'NYSE',BLK:'NYSE',C:'NYSE',AXP:'NYSE',SCHW:'NYSE',USB:'NYSE',
  PNC:'NYSE',TFC:'NYSE',PFE:'NYSE',ABT:'NYSE',CVS:'NYSE',MDT:'NYSE',
  ISRG:'NASDAQ',COP:'NYSE',EOG:'NYSE',SLB:'NYSE',MPC:'NYSE',
  PSX:'NYSE',VLO:'NYSE',OXY:'NYSE',DVN:'NYSE',HAL:'NYSE',BKR:'NYSE',
  T:'NYSE',VZ:'NYSE',MO:'NYSE',SPG:'NYSE',AMT:'NYSE',PLD:'NYSE',
};
  ...
  return (
    <div style={{ display:'flex', gap:4 }}>
      {Array.from({ length:max }).map((_,i) => {
        ...
        return <div key={i} style={{ width:7,height:7,borderRadius:'50%', background:filled?color:'transparent', border:'1px solid '+(filled?color:C.borderDk) }}/>;
      })}
    </div>
  );
}
...
function FeatureCard({ stock, rank }) {
  ...
  var sc=Math.min(stock.score||0,6), medals=['I','II','III'];
...
function ResultCard({ stock, rank }) {
  var sc=Math.min(stock.score||0,6), rating=getRating(sc);
  var chgPos=stock.change&&stock.change.startsWith('+');
  var exchange=getExchange(stock);
...
// Animated ellipsis for computing state
function ComputingDots() {
...
export default function Home() {
  ...
  var [top3State,setTop3State]       = useState('loading'); // 'loading' | 'computing' | 'ready'
  var [elapsed,setElapsed]           = useState(0);
  var timerRef    = useRef(null);
  var pollRef     = useRef(null);
  var elapsedRef  = useRef(null);
  var tickersRef  = useRef([]);
 
  // Poll /api/top3 until we get real data
  var fetchTop3 = useCallback(function() {
    fetch('/api/top3')
  ...
  useEffect(function() {
    fetchTop3();
    // Poll every 3 seconds while computing
    pollRef.current = setInterval(fetchTop3, 3000);
    // Elapsed timer for UX
    elapsedRef.current = setInterval(function() { setElapsed(function(e) { return e + 1; }); }, 1000);
    return function() { clearInterval(pollRef.current); clearInterval(elapsedRef.current); };
  }, [fetchTop3]);
  ...
  var scan = useCallback(function(tickers) {
    ...
    return fetch('/api/analyse', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({tickers}) })
      ...
      .finally(function() { setScanning(false); setTimeout(function() { setProgress(0); }, 1000); });
  }, []);
        ...
        <div style={{ maxWidth:1200,margin:'0 auto',padding:'32px 32px 80px' }}>
          ...
          <div style={{ marginBottom:40 }}>
            <div style={{ display:'flex',alignItems:'baseline',gap:16,marginBottom:20 }}>
              ...
              <div style={{ fontSize:10,color:C.txLight,fontFamily:SANS,letterSpacing:'0.1em',textTransform:'uppercase',whiteSpace:'nowrap' }}>
                {top3State==='ready'&&scannedTotal>0?scannedTotal+' securities screened':''}
              </div>
            </div>
 
            {/* Shimmer placeholders while loading/computing */}
            {top3State !== 'ready' && (
              <div>
                ...
                {top3State === 'computing' && (
                  <div style={{ textAlign:'center',fontSize:12,color:C.txLight,fontFamily:MONO,letterSpacing:'0.08em',animation:'pulse 1.5s infinite' }}>
                    Scanning {elapsed > 0 ? elapsed + 's' : ''} — analysing securities<ComputingDots/>
                  </div>
                )}
              </div>
            )}
            ...
            {/* Cards */}
            {top3State === 'ready' && top3Stocks.length > 0 && (
          ...
          <div style={{ background:C.cardBg,border:'0.5px solid '+C.borderDk,padding:'20px',marginBottom:20 }}>
            <div style={{ display:'flex',gap:10,marginBottom:14 }}>
              <input type="text" value={input} onChange={function(e){setInput(e.target.value);}} onKeyDown={function(e){if(e.key==='Enter')runScan();}}
